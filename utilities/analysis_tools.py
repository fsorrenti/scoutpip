...
"""plotting using get_dist"""


"""I add this just for having not filled constours"""
...
def eigenvalue_1_not_working(a11, a12, a13, a22, a23):
    q = 3 * (a11**2 + a11 * a22 + a12**2 + a13**2 + a22**2 + a23**2).real
    r = (
        27
        * (
            a11**2 * a22 / 2
    ...
    s = csqrt_vectorize(-4 * q**3 + (r + 2 * I * q**1.5) ** 2).real
    t = (csqrt_vectorize(s / 2 - r / (3 * 2 ** (1 / 3)) - q / (3 * 2 ** (2 / 3)))).real
    return -t - (q / (3 * t) + r / (3 * t) + 2 ** (1 / 3) * (a11 + a22 + a23)) / 3


def eigenvalue_2_not_working(a11, a12, a13, a22, a23):
    a = -3 * (a11**2 + a11 * a22 + a12**2 + a13**2 + a22**2 + a23**2).real
    b = (
        27 * a11**2 * a22 / 2
        - 27 * a11 * a12**2 / 2
        + 27 * a11 * a22**2 / 2
    ...
    c = (-4 * a**3 + (b + 2 * I * a**1.5) ** 2).real
    sqrt_c = csqrt_vectorize(c).real
    s = (sqrt_c / 2 - b / (3 * 2 ** (1 / 3)) - a / (3 * 2 ** (2 / 3))).real
    t = csqrt_vectorize(s) - b / (3 * csqrt_vectorize(s)) - a / (3 * csqrt_vectorize(s))
    return t.real


def eigenvalue_3_not_working(a11, a12, a13, a22, a23):
    q = (3 * (a11**2 + a11 * a22 + a12**2 + a13**2 + a22**2 + a23**2)).real
    r = (
        27
        * (
    ...
    t = (csqrt_vectorize(s / 2 - r / (3 * 2 ** (1 / 3)) - q / (3 * 2 ** (2 / 3)))).real
    return (
        -t - (q / (3 * t) + r / (3 * t) + 2 ** (1 / 3) * (a11 + a22 + a23)) / 3
    ).real
